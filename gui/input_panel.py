...
Input Panel
===========
Handles the process input form.
Dynamically shows/hides fields based on scheduler type.
Validates user input before passing to the scheduler.
"""
...
class InputPanel(tk.LabelFrame):
    """Panel for entering process information."""
    ...
    def _build_empty_state(self):
        """Show placeholder message before setup."""
    ...
    def setup_inputs(self, num_processes, needs_priority=False):
        """
        Create input fields for processes.

        Args:
            num_processes: number of process rows to create
            needs_priority: whether to show the priority column
        """
    ...
    def get_process_data(self):
        """
        Read and validate all process inputs.

        Returns:
            List of tuples: [(pid, arrival, burst, priority), ...]
    ...
    def clear(self):
        """Reset the input panel to empty state."""
        for widget in self.winfo_children():
            widget.destroy()
        self.process_entries.clear()
        self._build_empty_state()